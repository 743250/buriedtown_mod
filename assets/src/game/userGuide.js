...
var userGuide = {
    ...
    initUserGuide: function () {
        if (!Record.needUserGuide() && this.isStep(this.stepName.GAME_START)) {
            player.setSetting("step", this.stepName.HOME_GATE_AGAIN + 1);
            //player.room.createBuild(1, 0);
            //player.room.createBuild(14, 0);
            //player.room.createBuild(9, 0);
        }
        Record.closeUserGuide();
    },
    ...
    init: function () {
        ...
        if (str) {
            this.equipGuideList = SafetyHelper.safeJSONParse(str, null, "userGuide.init");
        } else {
            this.equipGuideList = {
