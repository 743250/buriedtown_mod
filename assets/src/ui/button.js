...
var SwitchMusicButton = CheckBox.extend({
    ctor: function (on) {
        this._super(on, "icon_music_on.png", "icon_music_off.png");
    },
    updateView: function () {
        this.getChildByName("checked").setVisible(!this.on);
    }
...
var ButtonAtChooseScene = Button.extend({
    ctor: function (spriteName, fallbackName) {
        ...
        var sprite = null;
        if (typeof uiUtil !== "undefined" && uiUtil && typeof uiUtil.getSpriteByNameSafe === "function") {
            sprite = uiUtil.getSpriteByNameSafe(spriteName, fallbackName || null);
        } else {
            sprite = SafetyHelper.safeLoadSprite(spriteName, fallbackName || null);
        }
        if (!sprite) {
