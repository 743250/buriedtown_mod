...
var EquipNode = cc.Node.extend({
    ...
    _getContentIconName: function(itemId) {
        if (itemId === Equipment.HAND) {
            return "#icon_tab_content_hand.png";
        }
        var displayId = uiUtil.getDisplayItemId(itemId);
        return "#icon_tab_content_" + displayId + ".png";
    },

    // 获取槽位图标名称
    _getTabIconName: function(itemId, equipPos) {
        if (itemId) {
            if (itemId === Equipment.HAND) {
                return "#icon_tab_hand.png";
            }
            var displayId = uiUtil.getDisplayItemId(itemId);
            return "#icon_tab_" + displayId + ".png";
        }
    ...
    createOneLineView: function (itemId) {
        ...
        if (itemId) {
            var iconName = this._getContentIconName(itemId);
            var itemInfo;
            if (itemId === Equipment.HAND) {
                itemInfo = {
                ...
                itemInfo = utils.clone(itemConfig[itemId]);
                itemInfo.name = stringUtil.getString(itemId).title;
                if (typeof TalentService !== "undefined" && TalentService && TalentService.applyElitePistolWeaponEffect) {
