...
var SiteNode = BottomFrameNode.extend({
    ...
    _init: function () {
        ...
        var txt1 = new cc.LabelTTF(
            cc.formatStr(template.title.txt_1, this.site.getProgressStr()),
        ...
        txt1.setName("site_progress_label");
        this.bg.addChild(txt1);

        var txt2 = new cc.LabelTTF(
            cc.formatStr(template.title.txt_2, this.site.storage.getAllItemNum()),
            uiUtil.fontFamily.normal,
            uiUtil.fontSize.COMMON_3
        );
        txt2.setAnchorPoint(1, 1);
        txt2.setPosition(rightEdge + 20, this.actionBarBaseHeight - 4);
        txt2.setName("site_storage_label");
        ...
        var des = new cc.LabelTTF(
            this.site.getDes(),
            uiUtil.fontFamily.normal,
            uiUtil.fontSize.COMMON_2,
            cc.size(rightEdge - leftEdge, 0)
        );
    ...
    refreshSiteHeaderStats: function () {
        ...
        var progressLabel = this.bg.getChildByName("site_progress_label");
        var storageLabel = this.bg.getChildByName("site_storage_label");
        if (progressLabel && titleTemplate.txt_1) {
            progressLabel.setString(cc.formatStr(titleTemplate.txt_1, this.site.getProgressStr()));
        }
        if (storageLabel && titleTemplate.txt_2) {
            storageLabel.setString(cc.formatStr(titleTemplate.txt_2, this.site.storage.getAllItemNum()));
        }
        if (this.notifyIcon) {
            this.notifyIcon.setVisible(this.site.haveNewItems);
        }
