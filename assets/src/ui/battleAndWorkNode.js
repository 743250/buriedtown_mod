...
var BattleAndWorkNode = BottomFrameNode.extend({
    ...
    _init: function () {
        ...
        des.setName("des");
        des.setColor(UITheme.colors.TEXT_NORMAL);
    ...
    createBattleProcessView: function () {
        ...
        for (var i = 0; i < 7; i++) {
            if (cc.sys.os === cc.sys.OS_ANDROID) {
                ...
                var label = new cc.LabelTTF("", uiUtil.fontFamily.normal, uiUtil.fontSize.COMMON_3, cc.size(node.width, uiUtil.fontSize.COMMON_3));
            }
                ...
                for (var i = 6; i >= 0; i--) {
                    ...
                    if (currentLogInfo) {
                        ...
                        if (currentLogInfo.color) {
                            ...
                            label.setColor(UITheme.colors.TEXT_NORMAL);
                        }
        ...
        labelNum.setColor(UITheme.colors.TEXT_NORMAL);
    ...
    _applyScavengerBattleLoot: function (sumRes) {
        if (!sumRes || !sumRes.win) {
            return;
        }
        var produceList = null;
        if (this.site.isInSecretRooms && this.site.secretRoomsConfig && this.site.secretRoomsConfig.produceList) {
            produceList = this.site.secretRoomsConfig.produceList;
        }
        if ((!produceList || produceList.length === 0) && this.site.config && this.site.config.produceList) {
            produceList = this.site.config.produceList;
        }
        if (!produceList || produceList.length === 0) {
            return;
        }

        var baseValue = this.room && this.room.difficulty ? parseInt(this.room.difficulty) : 1;
        baseValue = Math.max(1, Math.floor(baseValue / 2));
        var battleLoot;
        if (typeof SiteRewardService !== "undefined" && SiteRewardService && SiteRewardService.buildBattleScavengerLoot) {
            battleLoot = SiteRewardService.buildBattleScavengerLoot({
                isWin: sumRes.win,
                produceList: produceList,
                baseValue: baseValue
            });
        } else if (TalentService.hasChosenTalent(122)) {
            var isDouble = TalentService.rollScavengerDoubleDrop();
    ...
    createBattleEndView: function (sumRes) {
        ...
        label5.setPosition(0, label2.getPositionY() - label2.getContentSize().height - 10);
        node.addChild(label5);

        var bottomY = label5.getPositionY() - label5.getContentSize().height - 10;
        var label4;
        var richText4;
        if (sumRes.scavengerLootItems && sumRes.scavengerLootItems.length > 0) {
            var lootName = sumRes.scavengerDoubleTriggered ? "战斗搜刮(双倍)" : "战斗搜刮";
            label4 = new cc.LabelTTF(lootName + ":", uiUtil.fontFamily.normal, uiUtil.fontSize.COMMON_3);
            label4.setAnchorPoint(0, 1);
            label4.setPosition(0, bottomY);
            node.addChild(label4);

            richText4 = new ItemRichText(sumRes.scavengerLootItems, this.rightEdge - this.leftEdge - label4.width, 5, 0.5);
            richText4.setName("richText4");
            richText4.setAnchorPoint(0, 0.5);
            richText4.setPosition(label4.x + label4.width, label4.y - label4.height / 2);
            node.addChild(richText4);

            bottomY = label4.getPositionY() - label4.getContentSize().height - 10;
        }
        ...
        if (cc.RTL) {
            ...
            if (richText4) {
                richText4.anchorX = 1;
                richText4.x = label4.x - label4.width;
            }
        ...
        if (userGuide.isStep(userGuide.stepName.NEXT_ROOM)) {
            ...
            if (cc.sys.isNative && cc.sys.os == cc.sys.OS_IOS) {
                CommonUtil.afOnFinishFirstGuide();
            }
        }
